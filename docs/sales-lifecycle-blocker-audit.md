...
## Gift shortcut follow-up

- The existing catalog `giftItem` checkbox now explicitly controls inclusion in the order-builder Gift shortcuts list. All eligible marked gifts are exposed rather than only the first ten or only those below a cart-derived profit allowance.
...
- The known Titan hat Books item `1254360000043727500` remains an exact-ID fallback when bounded catalog data omits it or its local gift flag is stale. No provider write or transaction is part of this change.
- The order preview now shows the full available billing/shipping address and truthfully labels fulfillment as unselected, shipping as not calculated, and tax as provider-calculated. The displayed amount is a document subtotal before those later charges, preventing a draft preview from masquerading as a final landed total.
