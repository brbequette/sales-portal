...
Latest acceptance supersedes the earlier unmatched-event limitation below: PR #124 is merged and production deploy `6ab6a488f973dd00083360e1` is published at combined source `4a45432d`. Signed dashboard test event `cmuh77be90001skeq8gca69pd` is durably stored without an account association and visible in the admin inbox. All 42 focused voice/CRM tests passed on combined source. Existing Retell evidence remains one row. Shared Zoho OAuth cache was refreshed only after CRM/Books/Inventory verification; cached Calls field access now returns HTTP 200. Fresh real-call/transfer acceptance and remaining caller/notice/CRM-upload work are not complete. Full evidence is in `docs/retell-production-acceptance-20260925.md`.
