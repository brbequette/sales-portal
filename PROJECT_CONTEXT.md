...
## Invoice calculation verification (in progress, 2026-09-25)
...
- Invoice-only reconciliation is active with a durable 14,999-provider-call cap. At the latest checkpoint, 5,939 invoices passed independent arithmetic and persisted-field verification with zero mismatches; 3,302 provider calls had been used. Remaining source reads and unresolved payment/cost/salesperson evidence prevent certifying completion. Offline payment methods are not assumed to be card or cash.
...
- Validation: focused fee/history/reconciliation/persistence tests (62) and TypeScript pass; full isolated Next webpack production build passed with a temporary build-only auth secret. The additional concurrency guard test passes. Prisma regeneration was skipped for that build because the active reconciliation reader holds the shared Windows Prisma DLL; schema is unchanged.
