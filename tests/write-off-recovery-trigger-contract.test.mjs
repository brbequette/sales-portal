...
const observabilityMigration = fs.readFileSync("prisma/migrations/20260917110000_write_off_anomaly_observability/migration.sql", "utf8")
...
assert.match(trigger, /ANOMALY_SCHEMA_VERSION = "SANITIZED_V1"/)
assert.doesNotMatch(trigger, /customer_name.*sanitizedEvidence|customer_name.*diagnostics/)
...
assert.doesNotMatch(observabilityMigration, /\bUPDATE\b|\bDELETE\b/i)
assert.match(observabilityMigration, /ADD COLUMN "anomalySchemaVersion" TEXT/)
