  ...
  try {
    ...
    if (event.httpMethod === "PATCH" || event.httpMethod === "POST") {
      ...
      if (!isAdminRole(sessionUser.role) && (!actorId || account.ownerId !== actorId)) {
        return { statusCode: 403, headers: cors, body: JSON.stringify({ success: false, error: "Forbidden: This account belongs to another representative" }) }
      }
      ...
      const author = actorId ? await prisma.user.findUnique({ where: { id: actorId } }) : null
      if (author) {
