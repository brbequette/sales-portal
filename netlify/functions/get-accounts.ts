...
export const handler: Handler = async (event, context) => {
  ...
  if (email && adminEmailAliases[email.toLowerCase()]) {
    ...
    // PERF: single $queryRaw with SQL aggregation replaces two findMany(invoices:{items:true}) calls.
    // SUM/COUNT/json_agg run inside PostgreSQL — no invoice arrays or items blobs cross the wire.
    // Response shrinks from ~2-6MB to ~80KB per page.

    // Role/owner scoping as SQL fragments
    let scopeSql: any = Prisma.empty
    if (isSalesOnly && user) {
      const ownerIds = [user.id, user.zohoId, user.email].filter(Boolean) as string[]
      scopeSql = Prisma.sql`AND a."ownerId" IN (${Prisma.join(ownerIds)})`
      ...
      if (matchingUsers.length > 0) {
        ...
        scopeSql = Prisma.sql`AND a."ownerId" IN (${Prisma.join(ids)})`
      }
    }

    const statusSql: any = statusFilter
      ? Prisma.sql`AND a.status = ${statusFilter}`
      : Prisma.sql`AND a.status NOT IN ('Inactive','Do Not Contact','DNR')`

    const searchSql: any = search
      ? Prisma.sql`AND a.name ILIKE ${'%' + search + '%'}`
      : Prisma.empty

    const INV_EXCL = ['Void','void','Voided','voided','Draft','draft','Writeoff','Write_off','Write Off','Bad Debt','writeoff','write_off','write off','bad debt']

    // Count for pagination (fast, no joins)
    ...
    // Main query — all invoice metrics aggregated in SQL, zero items blobs cross the wire
    const dbAccounts: any[] = await prisma.$queryRaw<any[]>(Prisma.sql`
      SELECT
        ...
        COALESCE(SUM(i.amount) FILTER (WHERE i.status NOT IN (${Prisma.join(INV_EXCL)})), 0)::float AS "totalSales",
        COALESCE(SUM(COALESCE(i."computedProfit", (i.items->>'profit')::float))
          FILTER (WHERE i.status NOT IN (${Prisma.join(INV_EXCL)})), 0)::float AS "totalProfit",
        COALESCE(SUM(COALESCE(i.balance, i.amount)) FILTER (WHERE i.status ILIKE '%overdue%'), 0)::float AS "overdueBalance",
        COALESCE(COUNT(i.id) FILTER (WHERE i.status ILIKE '%overdue%'), 0)::int AS "overdueCount",
        COALESCE(SUM(COALESCE(i.balance, i.amount))
          FILTER (WHERE i.status NOT IN (${Prisma.join(INV_EXCL)}) AND i.status <> 'Paid' AND COALESCE(i.balance, i.amount) > 0),
          0)::float AS "unpaidBalance",
        COALESCE(COUNT(i.id)
          FILTER (WHERE i.status NOT IN (${Prisma.join(INV_EXCL)}) AND i.status <> 'Paid' AND COALESCE(i.balance, i.amount) > 0),
          0)::int AS "unpaidCount",
        MAX(i."issueDate") FILTER (WHERE i.status NOT IN (${Prisma.join(INV_EXCL)})) AS "latestInvoiceDate",
        COALESCE(json_agg(json_build_object(
          ...
          WHERE i.status NOT IN (${Prisma.join(INV_EXCL)}) AND i.status <> 'Paid' AND COALESCE(i.balance, i.amount) > 0
    ...
    // Map flat rows → response shape, no JS aggregation needed
