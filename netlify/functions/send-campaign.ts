  ...
  try {
    ...
    if ((channel || 'SMS') !== 'SMS') {
      return {
        statusCode: 501,
        headers: corsHeaders,
        body: JSON.stringify({ success: false, message: `${channel} campaign provider is not configured; nothing was sent or logged` })
      }
    }
    ...
    let resultMessage = `Campaign chunk sent successfully to ${successfulCount} customers. Failed: ${failedCount}`
