...
describe("SMS send surfaces", () => {
  ...
  it("makes individual sends duplicate-safe and preserves unknown provider outcomes", () => {
    const source = read("netlify/functions/send-sms.ts")
    expect(source).toContain("zoho-voice:sms:individual:${String(requestId)}")
    expect(source).toContain("requestFingerprint")
    expect(source).toContain("state: 'AMBIGUOUS'")
    expect(source).toContain("providerCode === 'ZVSMS-2000'")
  })
