...
const authenticatedHandler: Handler = async (event) => {
  ...
  try {
    ...
    const note = await prisma.$transaction(async tx => {
      const savedNote = await tx.note.create({
        data: {
      ...
      await tx.communicationEvent.create({
        data: {
          ...
          channel: "CALL",
          direction: "OUTBOUND",
          eventType: "SALES_CALL_LOGGED",
          sourceType: "Note",
          sourceId: savedNote.id,
          subject: outcomeLabels[outcome] || outcome || "Sales outreach",
          summary: fullContent.slice(0, 1000),
          occurredAt: savedNote.createdAt,
          metadata: {
            contactReached: Boolean(contactReached),
