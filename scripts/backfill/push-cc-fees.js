...
async function getZohoAccessToken() {
  ...
  if (tokenSetting) return tokenSetting.value;
...
async function run() {
  ...
  for (const inv of allInvoices) {
    if (!inv.items || !inv.items.custom_fields) continue;
    const cf = inv.items.custom_fields.find(c => c.label && c.label.toUpperCase().includes('CREDIT CARD PROCESSING'));
  ...
  for (const inv of toUpdate) {
    ...
    if (pushedCount % 100 === 0) console.log(`Pushed ${pushedCount}...`);
