...
const basePlan = JSON.parse(await fs.readFile("outputs/price-guide-db-images/update-plan-v3.json", "utf8"));
...
const promoted = basePlan.productUpdates.map((row) => {
  ...
  const newImageUrl = `/product-images/pioneer-price-guide/transparent-v4/price-guide-${match[1].toLowerCase()}.png`;
...
const output = { ...basePlan, version: "transparent-v4-ai-segmented", productUpdates: promoted };
await fs.writeFile("outputs/price-guide-db-images/update-plan-v4.json", JSON.stringify(output, null, 2), "utf8");
