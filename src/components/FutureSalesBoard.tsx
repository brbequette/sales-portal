...
export function FutureSalesBoard() {
  ...
  if (!data || !metrics) return <div className={styles.loading}><FiAlertTriangle />{refreshError === "unauthorized" ? "Dashboard authorization required" : refreshError === "incomplete" ? "Dashboard data incomplete" : refreshError === "network" ? "Dashboard network failure" : "Sales data unavailable"}</div>
  ...
  return <div ref={boardRef} className={styles.board}>
    <header className={styles.header}>
      ...
      <div className={styles.live}><i className={refreshError ? styles.warn : ""} />{refreshError ? `STALE / ${refreshError.toUpperCase()}` : "LIVE LOCAL DATA"}<span>UPDATED {updated}</span><small>{data.dataContract?.dateRange} · {data.dataContract?.timezone}</small></div>
