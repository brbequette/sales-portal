...
export function FutureSalesBoard() {
  ...
  return <div ref={boardRef} className={styles.board}>
    ...
    <section className={styles.weekBoard} aria-label="Weekly sales by representative and day">
      ...
      <div className={styles.weekBody}>{metrics.reps.slice(0, 7).map((rep: any) => <div className={styles.weekRow} key={rep.id}><div className={styles.weekRep}><i>{String(rep.name || "?").charAt(0)}</i><strong>{rep.name}</strong></div>{[0,1,2,3,4].map(day => { const daySales = Number(rep.goalDailySales?.[day] || 0); const dayProfit = Number(rep.goalDailyProfit?.[day] || 0); const dayCount = Number(rep.goalDailyCount?.[day] || 0); const pendingCosts = Number(rep.goalDailyPending?.[day] || 0); const dayMargin = daySales > 0 ? dayProfit / daySales * 100 : 0; const isLeader = daySales > 0 && daySales === metrics.dailyLeaders[day]; return <div className={`${styles.weekDay} ${isLeader ? styles.dayLeader : ""}`} key={day}>{isLeader && <em>★ DAY LEADER</em>}<strong>{daySales ? money(daySales, true) : "—"}</strong>{pendingCosts > 0 ? <small className={styles.costPending}>PROCESSING {pendingCosts} COST{pendingCosts === 1 ? "" : "S"}</small> : daySales ? <small className={styles.dailyMeta}><span>{dayCount} DOC</span><span>{money(dayProfit, true)} DP · {pct(dayMargin)}</span></small> : <small>NO SALES</small>}</div>})}<div className={`${styles.weekDay} ${styles.weekTotal}`}><strong>{money(rep.weekSales, true)}</strong><small>{rep.goalDailyPending.some((value: number) => value > 0) ? "COSTS PROCESSING" : `${money(rep.weekProfit, true)} DEAD PROFIT`}</small></div></div>)}</div>
