...
export async function GET() {
  ...
  const sampleSqlNull = await tryQuery("sampleSqlNull", `SELECT id, "zohoId", "invoiceNumber", items FROM "Invoice" WHERE items->'line_items' IS NULL LIMIT 1`)
  const sampleSqlEmpty = await tryQuery("sampleSqlEmpty", `SELECT id, "zohoId", "invoiceNumber", items FROM "Invoice" WHERE items->'line_items' = '[]'::jsonb LIMIT 1`)

  // Let's also retrieve the first 5 invoices to see their JSON keys at SQL level
  const first5Invoices = await tryQuery("first5", `SELECT id, "invoiceNumber", jsonb_object_keys(items) as key FROM "Invoice" LIMIT 20`)
