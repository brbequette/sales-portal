...
export async function GET() {
  try {
    ...
    const res1 = await fetch(url1, { signal: AbortSignal.timeout(15000), headers: { Authorization: `Zoho-oauthtoken ${t}` } });
    const data1 = await res1.json();

    const url2 = `https://www.zohoapis.com/books/v3/invoices?organization_id=${org1}&invoice_number=8189`;
    const res2 = await fetch(url2, { signal: AbortSignal.timeout(15000), headers: { Authorization: `Zoho-oauthtoken ${t}` } });
    const data2 = await res2.json();

    const pdfUrl = `https://www.zohoapis.com/books/v3/invoices/1254360000018829558?organization_id=${org1}&accept=pdf`;
    const resPdf = await fetch(pdfUrl, { signal: AbortSignal.timeout(15000), headers: { Authorization: `Zoho-oauthtoken ${t}` } });
