...
export default function BackfillPage() {
  ...
  // Safe JSON parse â€” guards against empty body (e.g. function timeout)
  const safeJson = async (res: Response): Promise<any> => {
    const text = await res.text()
    if (!text || text.trim() === '') throw new Error(`Empty response (HTTP ${res.status}) â€” function may have timed out`)
  ...
  const runPhase1 = async () => {
    ...
    addLog("â–¶ Phase 1 starting â€” processing one Zoho page per call, auto-continuing...")
      ...
      try {
        ...
        addLog(
          data.done
            ...
            : `ðŸ“„ ${data.module} pg ${data.page}: +${data.pageMapped} mapped Â· ${data.percentComplete}% done`
        )
    ...
    try {
      ...
      addLog(
        data.done
          ...
          : `âœ… Batch: +${data.batchProcessed} cached (${data.batchErrors || 0} errors) Â· ${data.percentComplete}% Â· ${data.remaining} left Â· ~${data.etaMinutesRemaining}m remaining`
      )
      return data
    } catch (e: any) {
      ...
      try {
        ...
        if (data.error) {
          ...
          if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
            addLog(`â›” ${MAX_CONSECUTIVE_ERRORS} consecutive errors â€” stopping. Click Resume to try again.`)
            break
          }
        ...
        addLog(
          data.done
            ? `ðŸŽ‰ DONE! All records backfilled.`
            : `âœ… Batch: +${data.batchProcessed} cached (${data.batchErrors || 0} errors) Â· ${data.percentComplete}% Â· ${data.remaining?.toLocaleString()} left Â· ~${data.etaMinutesRemaining}m remaining`
        )
    ...
    <div className="min-h-screen bg-neutral-950 text-white p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm mt-1">
            One-time operation to populate line items for all {status?.totals.total.toLocaleString() || 'â€¦'} local records
          </p>
        </div>
        <button
          onClick={fetchStatus}
      ...
      {status && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: "Total Records", value: status.totals.total.toLocaleString(), sub: `${status.totals.invoices} inv Â· ${status.totals.salesOrders} SO Â· ${status.totals.quotes} quotes`, color: "border-neutral-700" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={`bg-neutral-900 border rounded-xl p-5 space-y-3 ${phase1Done ? 'border-emerald-800' : 'border-neutral-700'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">~80 API calls Â· ~3 minutes</p>
              </div>
            </div>
          ...
          <p className="text-xs text-neutral-400 leading-relaxed">
            ...
            Required before Phase 2 â€” without it, Phase 2 needs 2 API calls per record instead of 1.
          </p>
          {status && (
            <div className="text-xs text-neutral-500 bg-neutral-800 rounded-lg px-3 py-2">
              Invoices with Books ID: <span className="text-white font-bold">{status.hasBookId.invoices.toLocaleString()}</span> / {status.totals.invoices.toLocaleString()} Â·
              SOs: <span className="text-white font-bold">{status.hasBookId.salesOrders.toLocaleString()}</span> / {status.totals.salesOrders.toLocaleString()} Â·
        ...
        <div className={`bg-neutral-900 border rounded-xl p-5 space-y-3 ${status?.checkpoint?.phase2Done ? 'border-emerald-800' : 'border-neutral-700'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">~15,500 API calls Â· 3â€“8 hours total Â· auto-continues</p>
              </div>
            </div>
          ...
          <p className="text-xs text-neutral-400 leading-relaxed">
            ...
            Rate-limited to 50 calls/min. Runs in batches of {18} â€” click Start and leave this tab open. Progress saves automatically.
          </p>
          ...
          {status?.checkpoint?.phase2Offset > 0 && !status?.checkpoint?.phase2Done && (
            <p className="text-xs text-sky-400">
              Checkpoint saved at offset {status?.checkpoint?.phase2Offset} â€” will resume from here.
            </p>
          )}
          <div className="flex gap-2">
            {!phase2Running ? (
      ...
      <div className="bg-amber-950/30 border border-amber-800/40 rounded-xl p-4 flex gap-3">
        ...
        <div className="text-xs text-amber-200/80 space-y-1">
          <p><strong>Run Phase 1 first</strong> â€” it maps 7,615 invoice Books IDs and will find SO/Quote IDs too. Takes ~3 minutes.</p>
          <p><strong>Phase 2 is safe to stop and restart</strong> â€” progress is saved after every batch of 18 records. Close the tab and come back anytime.</p>
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        setProgress(`Batch ${i + 1}/${totalChunks} done â€” ${totalUpdated} updated so far`)
      }
