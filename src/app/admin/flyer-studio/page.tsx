...
import { FiCheck, FiCloud, FiDownload, FiImage, FiLink, FiLoader, FiPackage, FiSave, FiSearch, FiUpload, FiZap } from "react-icons/fi"
...
async function renderFlyer(canvas: HTMLCanvasElement, width: number, height: number, product: SourceProduct, blade: Blade | null, rep: Rep | null, copy: FlyerCopy, reference?: string, promo: PromoForm = blankPromo, value = 0, savings = 0, backgroundArt?: string) {
  ...
  ctx.globalAlpha = 1
  const [logo, sourceImage, bladeImage, refImage, artImage] = await Promise.all([
    loadImage("/images/brand/titan-diamond-2026.png"), loadImage(product.imageUrl), loadImage(blade?.imageUrl), loadImage(reference), loadImage(backgroundArt),
  ])
  ...
  if (refImage) { ctx.save(); ctx.globalAlpha = .12; ctx.drawImage(refImage, 0, 0, width, height); ctx.restore() }
...
export default function FlyerStudioPage() {
  ...
  const [promo, setPromo] = useState<PromoForm>(blankPromo); const [promotionId, setPromotionId] = useState(""); const [publishing, setPublishing] = useState(false); const [previewKind, setPreviewKind] = useState<"sms" | "email">("sms"); const [generatedFlyer, setGeneratedFlyer] = useState(""); const [imageGenerating, setImageGenerating] = useState(false)
  ...
  function importProductScreenshot(files: FileList | null) {
    const file = files?.[0]
    if (!file) return
  ...
  return <div className="page-content flyer-studio">
    ...
    <div className="page-body grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(400px,.78fr)] gap-6 overflow-y-auto">
      <div className="space-y-5">
        <section className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><Step n="1" title="Choose or import the giveaway" icon={<FiLink />} /><label className="block text-xs font-bold text-neutral-400 mt-4">Existing Titan gift<select className="field mt-1" value={giftId} onChange={(e) => selectGift(e.target.value)}><option value="">Use a product link instead</option>{gifts.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.sku})</option>)}</select></label><div className="flex gap-2 mt-3"><input className="field flex-1" value={product.sourceUrl} onChange={(e) => setProduct({ ...product, sourceUrl: e.target.value })} placeholder="Paste Amazon, Home Depot, or Lowe's product URL" /><button className="btn-primary" onClick={scrape} disabled={scraping}>{scraping ? <FiLoader className="animate-spin" /> : <FiDownload />} Import</button></div><label className="btn-secondary mt-3 w-full justify-center cursor-pointer">{extractingScreenshot ? <FiLoader className="animate-spin" /> : <FiUpload />} {extractingScreenshot ? "Reading product screenshot..." : "Upload product-page screenshot"}<input className="hidden" type="file" accept="image/png,image/jpeg,image/webp" disabled={extractingScreenshot} onChange={(e) => { importProductScreenshot(e.target.files); e.target.value = "" }} /></label><p className="text-[11px] text-neutral-500 mt-2">If a retailer blocks the link, upload a screenshot showing the product title, price, image, and details. The screenshot becomes the giveaway image and its visible data fills the editable fields.</p>
